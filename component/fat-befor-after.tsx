...
const ThreeImageCarousel = () => {
  ...
  // Get visible images - with proper handling for partial slides
  const getVisibleImages = useCallback(() => {
    const startIndex = currentIndex * itemsPerView;
    const endIndex = Math.min(startIndex + itemsPerView, totalItems);
    const visibleImages = images.slice(startIndex, endIndex);
    
    // If we have fewer images than itemsPerView (last slide), add empty placeholders
    if (visibleImages.length < itemsPerView) {
      const placeholdersNeeded = itemsPerView - visibleImages.length;
      for (let i = 0; i < placeholdersNeeded; i++) {
        visibleImages.push({
          id: -i, // Negative ID for placeholders
          src: '',
          alt: 'placeholder'
        });
      }
  ...
  }, [currentIndex, itemsPerView, images, totalItems]);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [isAnimating, currentIndex, totalSlides, nextSlide, goToSlide]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-2 sm:px-4 md:px-6 py-8 md:py-12 max-sm:mt-22">
      ...
      {/* Carousel Container */}
      <div className="relative">
        {/* Navigation Buttons - Always visible but smaller on mobile */}
        <button
          onClick={prevSlide}
          disabled={isAnimating || currentIndex === 0}
            ...
            {screenSize === 'mobile' && (
              <div className="flex justify-center px-8 sm:px-4">
                {visibleImages.map((image) => (
                  <div key={image.id} className="w-full max-w-sm flex-shrink-0">
                    {image.src ? (
                      <div className="relative w-full h-92 sm:h-90 rounded-xl overflow-hidden shadow-lg">
                        <Image
                          src={image.src}
                          alt={image.alt}
                          fill
                      ...
                      <div className="relative w-full h-72 sm:h-80 rounded-xl overflow-hidden shadow-lg bg-gray-100 opacity-0" />
                    )}
                  </div>
                ))}
              </div>
                ...
                {visibleImages.map((image) => (
                  <div key={image.id} className="w-full">
                    {image.src ? (
                      ...
                      // Placeholder for empty slot
                      <div className="relative w-full h-64 md:h-72 rounded-xl overflow-hidden shadow-lg bg-gray-100 opacity-0" />
                    )}
                  </div>
                ))}
              </div>
                ...
                {visibleImages.map((image) => (
                  <div key={image.id} className="w-full">
                    {image.src ? (
                      ...
                      // Placeholder for empty slot
                      <div className="relative w-full h-72 md:h-80 lg:h-96 rounded-xl overflow-hidden shadow-lg bg-gray-100 opacity-0" />
                    )}
                  </div>
                ))}
              </div>
